Shader "CubedParadox/Flat Lit Toon Lite Fade MMD"
{
	Properties
	{
		...
		_SphereMulIntensity("Multiply Sphere Texture Intensity", Range(0, 1)) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(VertexOutput i) : COLOR
			{
				...
				float3 directContribution = saturate(1.0 + floor(saturate(remappedLight) * 2.0));

				float4 toonTexColor = tex2D(_ToonTex, float2(0.5, dot(lightDirection, normalDirection) * 0.5 + 0.5));
				float directionalLightBool = saturate(dot(_WorldSpaceLightPos0, _WorldSpaceLightPos0)); // 0 for no light, 1 for light
				toonTexColor = lerp(1.0, toonTexColor, directionalLightBool);

				float3 finalColor = emissive + ((baseColor + saturate(sphereAdd)) * sphereMul * lerp(indirectLighting, directLighting * toonTexColor, directContribution));
	...
	FallBack "Diffuse"
    CustomEditor "CubedsUnityShaders.FlatLitToonLiteCutoutInspectorMMD"
}